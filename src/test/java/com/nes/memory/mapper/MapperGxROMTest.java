...
/**
 * Unit tests for Mapper 66 (GxROM).
 */
class MapperGxROMTest {
    ...
    private MapperGxROM mapper;

    @BeforeEach
    void setUp() {
        mapper = new MapperGxROM(PRG_ROM, CHR_ROM, false);
    }
    ...
    class ChrBankSwitching {
        ...
        @Test
        void selectBank1() {
            mapper.cpuWrite(0x8000, 0x01);  // CHR bits 1-0 = 1
            assertEquals(1, mapper.ppuRead(0x0000) & 0xFF);
        }

        @Test
        void selectBank3() {
            mapper.cpuWrite(0x8000, 0x03);  // CHR bits 1-0 = 3
    ...
    class CombinedBankSwitching {
        ...
        void prgAndChrSetSimultaneously() {
            mapper.cpuWrite(0x8000, 0x12);  // PRG = 1, CHR = 2
            assertEquals(1, mapper.cpuRead(0x8000) & 0xFF);
        ...
        void independentSwitching() {
            mapper.cpuWrite(0x8000, 0x31);  // PRG = 3, CHR = 1
            assertEquals(3, mapper.cpuRead(0x8000) & 0xFF);
            assertEquals(1, mapper.ppuRead(0x0000) & 0xFF);
        }
    }
